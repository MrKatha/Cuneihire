...
## Deploy gotcha
Fixed 2026-08-28 — `.github/workflows/backend-deploy.yml` now actually auto-deploys: push to `master`/`main` touching `backend/**` (or `workflow_dispatch`) SSHs in, `git pull`, `npm i`, `pm2 restart auto_apply_linkedin_backend --update-env`, no test/lint gate. Secrets `SERVER_IP`/`SERVER_USER`/`SERVER_PORT`/`SERVER_PRIVATE_KEY` are set on the repo; the workflow used to also require `SERVER_ROOT_PASSWORD` for a `sudo` wrapper that turned out to be unnecessary (every manual deploy this whole project ran fine as the plain `ubuntu` user) — dropped, so there's one fewer secret to keep in sync. Verified live via a real `workflow_dispatch` run: git pulled, `npm i` ran, PM2 restarted, process came back online. `gh` note: this repo needs the `MrKatha` GitHub account, not `sohaib-axionai` — `gh auth switch --hostname github.com --user MrKatha` if `gh secret set`/similar 403s with "repository secrets fine-grained permission."
Manual SSH deploy (`git pull && npm i && pm2 restart auto_apply_linkedin_backend --update-env`) still works and is the fallback if the Action ever fails.
